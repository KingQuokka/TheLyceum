...
int main()
{
	...
	Font font;
	font.loadFromFile("fonts/MedievalSharp-Regular.ttf");
	...
	texturePlayer.loadFromFile("graphics/Kylith.png");
	Sprite spritePlayer;
	...
	Texture textureRIP;
	textureRIP.loadFromFile("graphics/riplol.png");
	Sprite spriteRIP;
	...
	// Prepare the axe
	Texture textureAxe;
	textureAxe.loadFromFile("graphics/HandOfKylith.png");
	Sprite spriteAxe;
	spriteAxe.setTexture(textureAxe);
	spriteAxe.setPosition(700, 750);
	// Line the axe up with the tree
	const float AXE_POSITION_LEFT = 700;
	const float AXE_POSITION_RIGHT = 1225;
	...
	while (window.isOpen())
	{
		...
		if (acceptInput)
		{
			...
			if (Keyboard::isKeyPressed(Keyboard::Right))
			{
				...
				spriteAxe.setScale(Vector2f(-1, 1));
		...
		if (!paused) 
		{
			...
			if (logActive)
			{
				...
				{
					// Setup a new log for the next frame
					logActive = false;
					spriteLog.setPosition(810, 720);
				}
			}
